...
function App() {
  ...
  <form>
    ...
    {
      isEdit === true ? (
         <button type="submit" onClick={todoEditQilish} className="btn btn-primary mt-3" style={{ width:"150px" }} >Save </button>
      ) :(
        ...
        data.map((value,index)=>{
           return(
            <tr key={value.id} scope="row">
              <td>{index + 1}</td>
              <td>{value.title}</td>
              <td> <button className="btn btn-warning" onClick={()=>editTing(value)}>edit</button> 
               <button className="btn btn-warning" onClick={()=>deleteQilish(value.id)}>delete</button>
              </td>
            </tr>
          )
        }
